...
class Solution {
    ...
    private TreeNode dfs(int[] preorder, int preStart, Map<Integer, Integer> inorderMap, int inStart, int inEnd) {
        ...
        // 3. 오른쪽 노드 찾기 - NLR이므로 preStart는 발견한 중위순회 idx(좌측노드 갯수) + 1부터 탐색, 중위순회에서는 시작 idx에서 좁혀간다.
        ...
        int leftSize = rootInIdx - inStart;         // 중위순회 배열에서 현재 root 노드의 인덱스 - 현재 중위순위 배열 시작값

        root.left = dfs(preorder, preStart + 1, inorderMap, inStart, rootInIdx - 1);
        // 전위 순회는 +1부터, 중위 순회는 end값 제한
        root.right = dfs(preorder, preStart + leftSize + 1, inorderMap, rootInIdx + 1, inEnd);
        // 전위 순회는 좌측 노드 갯수 + 1부터, 중위순회는 start값 제한
